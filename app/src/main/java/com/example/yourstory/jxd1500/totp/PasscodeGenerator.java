...
package com.example.yourstory.jxd1500.totp;
...
public class PasscodeGenerator {
    ...
    public static PasscodeGenerator getInstance() {
        if (mInstance == null) {
            ...
            SharedPreferences userSettings = MyApplication.getInstance().
                    getSharedPreferences("lxlpKey", 0);
            String name = userSettings.getString("lxlpKey", "abcdefg");
            AUTH_KEY = name;
        }
        return mInstance;
    }

    public static String generateTotpNum(long time) {
        return getInstance().generateResponseCode(time);
    }
    ...
    private String generateResponseCode(long time) {
        try {
...
//            long time = TimeUtils.getValueAtTime();
    ...
    public static void setAuthKey(String key) {
        getInstance().changeKey(key);
    }

    private void changeKey(String key) {
        AUTH_KEY = AlphaBetcode.Encode(key);
        SharedPreferences.Editor editor = MyApplication.getInstance().getSharedPreferences("key", MODE_PRIVATE).edit();
        editor.putString("key", AUTH_KEY);
        editor.apply();
    }
